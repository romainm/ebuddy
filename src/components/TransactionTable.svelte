<script>
  ...
  function wrap(ts) {
    return ts.map(t => {
      return {
        ...t,
        selected: selectedTransactionIds.includes(t._id)
      };
    });
  }
  ...
  $: selectedTransactionIds = selectedTransactions.map(t => t._id);
...
<div class="ui container" id="transactions-table">
    ...
    <tbody>
      {#each wrappedTransactions as transaction, i}
        <tr
          ...
          <td class="right aligned">{formatMoney(transaction.amount)}</td>
  ...
  <!-- <div class="ui right very close rail" class:invisible={lastSelected === null}>
    <div class="ui sticky">
